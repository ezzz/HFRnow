...
| HFR-042 | Beta p.19 | Images miniatures: tap simple ouvre une version illisible | UX / Bug media | Moyen | Moyenne | Correction appliquee: le viewer image dispose d'un resolver commun et le tap WebView intercepte les petites vignettes liees a une image grand format allowlistee. Premier site active: `img3.super-h.fr`, avec verification de taille <= 200x200, lien image et identite normalisee identique (`.th` retire). | Corrige, a tester |
| HFR-047 | Hors analyse initiale | AnswerView/MessagesView: clarifier reponse rapide, reponse forum et brouillons | UX / Fonction | Fort | Moyenne | Refonte du flux de reponse: reglage `Bouton +` a trois choix (`Reponse rapide`, `Repondre comme le forum`, `Pas d'icone`), action `Repondre` forum explicite dans le menu `...`, brouillon actif contextualise par topic, brouillons archives limites a trois par topic, archivage avant remplacement, sheet de brouillons avec action `Mettre de cote et vider`, suppression par swipe et action globale `Vider`. Finition: les titres de presentation sont francises (`Repondre`, `Edition`, `Nouv. message`). Specification ajoutee dans `specifications.md`. | Livré, a tester |
