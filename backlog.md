...
| HFR-031 | Hors analyse initiale | Filtrer les posts: le bandeau `Resultats suivants` ne fonctionne pas | Bug UX | Moyen | Faible | Dans la vue filtree `<3`, le clic sur `Resultats suivants` ne fait rien alors que la grosse fleche fonctionne. Option probable: supprimer le bandeau redondant puisque la grosse fleche est affichee en permanence. | A faire |
